...
{
    double  dummy_array[ncols];
    matrix_zero(dummy_array,ncols);
    // dummy_array = (double*) malloc(ncols * sizeof(double));
    //if (dummy_array == NULL) {printf(" No memory available for dummy array in SVD\n"); return -2; }
    ...
    if (Givens_Reduction_to_Diagonal_Form( nrows, ncols, U, V,
                                           singular_values, dummy_array ) < 0)
    {
        printf(" Failed to converge in SVD\n");
        return -1;
    }
